...
public class QualityAnalyzer
{
    ...
    private EntityMetrics AnalyzeEntitiesEnhanced(
        ...
        List<QualityIssue> issues)
    {
        if (summary.Entities == null || !summary.Entities.HasAny) return new EntityMetrics(0, 0, 0, 0, 0, [], []);
    ...
    private List<string> FindSuspiciousEntities(List<string> entities, string sourceText)
    {
        ...
        foreach (var entity in entities)
        {
            ...
            if (!normalizedSource.Contains(cleanEntity.ToLowerInvariant()))
            {
                ...
                if (foundWords < words.Length / 2.0) suspicious.Add(cleanEntity);
            }
        }
    ...
    private ClaimType ClassifyClaimType(string sentence, string sourceText)
    {
        ...
        var inferenceWords = new[]
        {
            "suggests", "implies", "indicates", "likely", "probably",
        ...
        var colourWords = new[]
        {
            "atmosphere", "setting", "mood", "tone", "style",
