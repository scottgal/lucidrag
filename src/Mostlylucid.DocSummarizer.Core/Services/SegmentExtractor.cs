...
public class SegmentExtractor : IDisposable
{
        ...
        CancellationToken ct = default)
    {
        ...
        if (_config.UseFastPreFilter && segments.Count > _config.MaxSegmentsToEmbed)
        {
            ...
            ProgressService.WriteVerboseMarkup(_verbose,
                $"[dim]Large document ({segments.Count} segments) - semantic pre-filtering to ~{targetEmbedCount}[/]");
        ...
        else if (segments.Count > _config.MaxSegmentsToEmbed)
        {
            ...
            ProgressService.WriteVerboseMarkup(_verbose,
                $"[dim]Large document ({segments.Count} segments) - using hierarchical extraction[/]");
        ...
        if (_verbose)
        {
            VerboseHelper.Log(_verbose,
                $"[dim]Extraction complete: {segments.Count} segments, top {topBySalience.Count} by salience[/]");
        ...
        CancellationToken ct)
    {
        ...
        if (globalCentroid != null)
        {
            ...
            if (norm > 0)
                for (var i = 0; i < globalCentroid.Length; i++)
        ...
        {
            VerboseHelper.Log(_verbose,
                $"[dim]Hierarchical extraction complete: {allSegments.Count} total, {batchWinners.Count} candidates, top {topBySalience.Count}[/]");
            VerboseHelper.Log(_verbose, $"[dim]Time: {stopwatch.Elapsed.TotalSeconds:F1}s[/]");
        }
    ...
    private List<Segment> ParseToSegments(string docId, string markdown, ContentType contentType)
    {
        ...
        foreach (var section in parsedDoc.Sections)
        {
            ...
            {
                var headingSegment = new Segment(docId, section.Heading, SegmentType.Heading, segments.Count,
                    charOffset, charOffset + section.Heading.Length)
                {
            ...
            if (_config.IncludeListItems)
                foreach (var item in section.ListItems)
                {
                    ...
                    var segment = new Segment(docId, item, SegmentType.ListItem, segments.Count, charOffset,
            ...
            foreach (var quote in section.Quotes)
            {
                ...
                var segment = new Segment(docId, quote, SegmentType.Quote, segments.Count, charOffset,
                    charOffset + quote.Length)
                {
                    SectionTitle = section.Heading,
                    HeadingPath = currentHeadingPath,
                    HeadingLevel = section.Level,
    ...
    public static ContentType DetectContentTypeFromSegments(List<Segment> segments)
    {
        ...
        // 2. Speech tags
        var speechTagPattern = new Regex(
        ...
        var narrativePattern = new Regex(
        ...
        {
            "function", "class", "method", "api", "http", "json",
        ...
        CancellationToken ct)
    {
        ...
        if (_verbose)
            VerboseHelper.Log(_verbose,
                $"[dim]Pass 1: Embedding {sample.Count} sample segments for topic anchors...[/]");
        ...
        var topicAnchors = ComputeTopicAnchors(embeddedSample, 5);
        ...
        var perSectionCap = Math.Max(10,
            maxToKeep / Math.Max(1, segments.Select(s => s.SectionTitle).Distinct().Count()));
    ...
    private HashSet<Segment> BuildGuaranteedCoverageSet(List<Segment> segments)
    {
        ...
        if (sentenceSections.Count > 50)
        {
            ...
            for (var i = 0; i < sentences.Count && guaranteed.Count < maxGuaranteed; i += step)
        ...
        {
            "dockerfile", "docker", "from ", "run ", "copy ", "cmd ",
    ...
    private static List<float[]> ComputeTopicAnchors(List<Segment> embeddedSample, int k)
    {
        ...
        while (anchors.Count < k)
        {
            ...
            for (var i = 0; i < embeddedSample.Count; i++)
            {
    ...
    private static List<Segment> SelectStratifiedSample(List<Segment> segments, int sampleSize)
    {
        ...
        foreach (var section in sections)
        {
            ...
            else
            {
                ...
                for (var i = 1; i < perSection - 1 && i * step < sectionList.Count - 1; i++)
        ...
        CancellationToken ct)
    {
        ...
        for (var batch = 0; batch < numBatches; batch++)
        {
            ...
            if (globalCentroid == null)
                ...
                for (var i = 0; i < globalCentroid.Length && i < batchCentroid.Length; i++)
            ...
            foreach (var segment in batchSegments.Where(s => !batchTop.Contains(s))) segment.Embedding = null;
        }
    ...
    private async Task GenerateEmbeddingsWithoutProgressAsync(List<Segment> segments, CancellationToken ct)
    {
        ...
        for (var i = 0; i < total; i += batchSize)
        {
            ...
            for (var j = 0; j < batch.Count; j++) batch[j].Embedding = embeddings[j];
        }
    }
    ...
    /// </summary>
    private void ComputeSalienceScores(List<Segment> segments, float[] centroid,
        ContentType contentType = ContentType.Unknown)
    {
    ...
    private double ComputeContentTypeWeight(Segment segment, ContentType contentType)
    {
        ...
        if (contentType == ContentType.Expository || contentType == ContentType.Unknown)
        {
            ...
            {
                var looksLikeCitation = Regex.IsMatch(
        ...
        var isDialogue = narrativeText.StartsWith('"') || narrativeText.StartsWith('"') ||
                         narrativeText.StartsWith("'");
        var isShort = narrativeText.Length < 60;
        var hasSimpleSpeechTag = Regex.IsMatch(
        ...
        if (!isDialogue)
        {
            ...
            var hasActionVerbs = Regex.IsMatch(
                narrativeText,
                @"\b(walked|ran|entered|left|opened|closed|found|discovered|saw|heard|felt|took|gave|came|went|arrived|departed|killed|died|attacked|escaped|hid|searched|followed)\b",
                RegexOptions.IgnoreCase);
            ...
            var hasSceneSetting = Regex.IsMatch(
                narrativeText,
                @"\b(morning|evening|night|day|room|house|street|door|window|chapter|later|earlier|meanwhile|suddenly|finally)\b",
                RegexOptions.IgnoreCase);
        ...
        var hasCharacterIntro = Regex.IsMatch(
