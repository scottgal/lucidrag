...
public class WebFetcher
{
        ...
        return new ResiliencePipelineBuilder<HttpResponseMessage>()
            ...
            .AddCircuitBreaker(new CircuitBreakerStrategyOptions<HttpResponseMessage>
            {
                ...
                OnOpened = args =>
                {
                    ...
                    Console.WriteLine(
                        $"[WebFetch] Circuit breaker OPENED - requests will fail fast for {args.BreakDuration.TotalSeconds:F0}s");
            ...
            .AddRetry(new RetryStrategyOptions<HttpResponseMessage>
            {
                ...
                DelayGenerator = args =>
                {
                    ...
                    if (args.Outcome.Result?.StatusCode == HttpStatusCode.TooManyRequests)
                    {
                        RateLimitCounter.Add(1,
                            new KeyValuePair<string, object?>("url.host",
                                args.Context.Properties.GetValue(new ResiliencePropertyKey<string>("host"),
                                    "unknown")));

                        var retryAfter = args.Outcome.Result.Headers.RetryAfter;
                        if (retryAfter?.Delta != null) return ValueTask.FromResult<TimeSpan?>(retryAfter.Delta.Value);
                ...
                OnRetry = args =>
                {
                    ...
                    Console.WriteLine(
                        $"[WebFetch] Retry {args.AttemptNumber}/{MaxRetryAttempts} after {args.RetryDelay.TotalSeconds:F1}s: {message}");
                    return ValueTask.CompletedTask;
                }
            })
            .Build();
        ...
        try
        {
            ...
            if (pixelArea > MaxImagePixelArea)
                throw new SecurityException(
                    $"Image too large: {image.Width}x{image.Height} ({pixelArea} pixels, max {MaxImagePixelArea})");
            ...
            if (!needsResize && !needsCompression) return (imageBytes, extension);
    ...
    private async Task ValidateHostAsync(Uri uri)
    {
        ...
        if (addresses.Length == 0) throw new SecurityException($"No addresses found for host: {uri.Host}");

        foreach (var address in addresses) ValidateIPAddress(address, uri.Host);
    }
    ...
    private async Task<WebFetchResult> FetchWithSecurityAsync(Uri originalUri)
    {
        ...
        while (true)
        {
            ...
            if (redirectCount > 0) await ValidateHostAsync(currentUri);
            ...
            try
            {
                ...
                response = await _resiliencePipeline.ExecuteAsync(
                    async token =>
                    {
                        ...
                        clonedRequest.Headers.Accept.ParseAdd(
                            "text/html, application/xhtml+xml, application/pdf, text/plain, text/markdown, application/json, image/png, image/jpeg, image/tiff, image/webp");

                        var resp = await httpClient.SendAsync(clonedRequest, HttpCompletionOption.ResponseHeadersRead,
                            token);
                            ...
                            if (PermanentFailureStatusCodes.Contains(resp.StatusCode))
                            {
                                var errorMessage = resp.StatusCode switch
                                {
                                    HttpStatusCode.Forbidden =>
                                        $"Access forbidden (403): {currentUri}. The server is blocking access - this may require authentication or the resource is restricted.",
                                    HttpStatusCode.Unauthorized =>
                                        $"Unauthorized (401): {currentUri}. Authentication is required.",
                                    HttpStatusCode.NotFound =>
                                        $"Not found (404): {currentUri}. The resource does not exist.",
                                    HttpStatusCode.Gone =>
                                        $"Gone (410): {currentUri}. The resource has been permanently removed.",
            ...
            if ((int)response.StatusCode >= 300 && (int)response.StatusCode < 400)
            {
                ...
                if (location == null) throw new InvalidOperationException("Redirect without Location header");
            ...
            if (!IsAllowedContentType(contentType)) throw new SecurityException($"Blocked content type: {contentType}");
    ...
    private async Task<WebFetchResult> ProcessResponseAsync(byte[] content, string contentType, Uri sourceUri)
    {
        ...
        if (IsBinaryDocument(extension))
        {
            ...
            return new WebFetchResult(tempFile, contentType, sourceUri.ToString(), extension);
        }
        ...
        if (IsImageContent(extension))
        {
            ...
            return new WebFetchResult(tempFile, contentType, sourceUri.ToString(), newExtension);
        }
        ...
        if (extension == ".html" || ct.Contains("html"))
        {
            ...
            return new WebFetchResult(tempFile, contentType, sourceUri.ToString(), ".html", true);
        }
        ...
        return new WebFetchResult(textTempFile, contentType, sourceUri.ToString(), extension);
    }
    ...
    private async Task<string> SanitizeHtmlAsync(string html, Uri baseUri)
    {
        ...
        foreach (var element in document.QuerySelectorAll("*").ToList())
        {
            ...
            foreach (var attr in attrsToRemove) element.RemoveAttribute(attr);
    ...
    private static void SanitizeUrlAttribute(IElement element, string attributeName)
    {
        ...
        if (colonIndex > 0 && colonIndex < 20) // Reasonable scheme length
        {
            ...
            if (DangerousSchemes.Contains(scheme)) element.RemoveAttribute(attributeName);
        }
    }
    ...
    private static async Task EnsurePlaywrightInstalledAsync()
    {
        ...
        try
        {
            ...
            if (exitCode != 0)
                throw new InvalidOperationException(
                    $"Failed to install Playwright Chromium browser (exit code {exitCode})");
    ...
    private async Task<WebFetchResult> FetchWithPlaywrightAsync(Uri uri)
    {
        ...
        try
        {
            ...
            if (!response.Ok) throw new HttpRequestException($"HTTP {response.Status}: {response.StatusText}");
            ...
            return new WebFetchResult(tempFile, contentType, finalUrl, ".html", true);
        }
